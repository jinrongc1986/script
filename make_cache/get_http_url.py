...
def sshclient_execmd(port, execmd, hostname='rhelp.fxdata.cn', username='root',
    ...
    stdin.write(
        "Y")  # Generally speaking, the first connection, \
    # need a simple interaction.
    output = stdout.read()
    s.close()
    return output
...
def get_port(sn):
    cmd = '''mysql -N -e "select rhelp from ordoac.feedback where \
            sn='CAS0530000152'"  -h 192.168.1.12 -uselector \
            -pfxdata_Select-2016 -P 3305'''
...
def geturls(sn='CAS0530000152'):
    ...
    child = pexpect.spawn(
        'scp -P %d  root@rhelp.fxdata.cn:/tmp/test_jinrongc.txt \
        ./urls.txt' % port)
